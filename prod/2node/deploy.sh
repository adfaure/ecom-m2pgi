...
echo "pull the git repository on vm3  "
	ssh vm3 "cd /home/im2ag/ECOM-M2PGI/ && git reset --hard HEAD && git pull"
echo "pull the git repository on vm5  "
	ssh vm5 "cd /home/im2ag/ECOM-M2PGI/ && git reset --hard HEAD && git pull"
echo "pull the git repository on vm6  "
	ssh vm6 "cd /home/im2ag/ECOM-M2PGI/ && git reset --hard HEAD && git pull"
...
echo "running nodejs second instance" 
if server_running 152.77.78.11 3000
then 
	echo "	node js is running on port 3000"
else 
		ssh -f vm6 '(export NVM_DIR="/home/im2ag/.nvm"
		[ -s "$NVM_DIR/nvm.sh" ] && . "$NVM_DIR/nvm.sh"  # This loads nvm &&
		nvm use 5.2.0 && cd /home/im2ag/ECOM-M2PGI/nodejs && node main.js)' &> $LOGDIR/deploy-node.log
fi
